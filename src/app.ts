...
export function createApp(dependencies: AppDependencies = {}) {
  ...
  registerReviewRoutes(app, {
    ...
    consentGateMw,
    getRequiredRouteParam,
    enforceIdentityBoundSignatures: dependencies.enforceIdentityBoundSignatures,
    signatureSealKey: dependencies.signatureSealKey,
  });
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        const correlationId = String(res.locals.correlationId ?? "");
        res.json({
          case: await store.registerSample(caseId, req.body, correlationId),
        });
      } catch (error) {
        next(error);
      }
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        const correlationId = String(res.locals.correlationId ?? "");
        res.json({
          case: await store.registerArtifact(caseId, req.body, correlationId),
        });
      } catch (error) {
        next(error);
      }
