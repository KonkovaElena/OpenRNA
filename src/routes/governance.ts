...
export function registerGovernanceRoutes(
  ...
  app.post("/api/cases/:caseId/consent", rbacAuth(rbacProvider, "REGISTER_SAMPLE"), async (req, res, next) => {
    try {
      ...
      if (currentCase.status === "CONSENT_WITHDRAWN" && input.type !== "withdrawn") {
        throw new ApiError(
          409,
      ...
      const newConsentStatus = input.type === "withdrawn" ? ("withdrawn" as const) : ("complete" as const);
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        const correlationId = String(res.locals.correlationId ?? "");
        const rationale =
          typeof req.body?.rationale === "string" && req.body.rationale.trim().length > 0
  ...
  app.get("/api/cases/:caseId/audit-chain/verify", rbacAuth(rbacProvider, "VIEW_CASE"), async (req, res, next) => {
    try {
      const caseId = getRequiredRouteParam(req, "caseId");
