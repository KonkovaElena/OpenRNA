...
export function registerAuditRoutes(
  ...
  app.post("/api/audit/sign", rbacAuth(rbacProvider, "ADMIN_OPERATIONS"), async (req, res, next) => {
    try {
      ...
      const signed = await auditSignatureProvider.signAuditEntry(
        input.entry as unknown as Parameters<typeof auditSignatureProvider.signAuditEntry>[0],
        input.principal,
      );
  ...
  app.post("/api/audit/verify", rbacAuth(rbacProvider, "VIEW_CASE"), async (req, res, next) => {
    try {
      ...
      const valid = await auditSignatureProvider.verifySignature(
        input.entry as unknown as Parameters<typeof auditSignatureProvider.verifySignature>[0],
      );
      res.json({ valid });
    } catch (error) {
      next(error);
