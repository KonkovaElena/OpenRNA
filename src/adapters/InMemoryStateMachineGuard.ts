...
import { type CaseStatus, caseStatuses } from "../types";
...
const STRICTLY_TERMINAL_STATES = new Set<CaseStatus>(["HANDOFF_PENDING", "REVIEW_REJECTED", "CONSENT_WITHDRAWN"]);
...
function allowedTransitionsFor(fromStatus: CaseStatus): CaseStatus[] {
  ...
  if (CONSENT_WITHDRAWAL_SOURCES.has(fromStatus) && !base.includes("CONSENT_WITHDRAWN")) {
