...
test("RBAC Provider", async (t) => {
  ...
  await t.test("canAccessCase enforces explicit case grants and admin bypass", async () => {
    const provider = new InMemoryRbacProvider({ allowAll: false });
    await provider.assignRole("owner-1", "OPERATOR");
    await provider.assignRole("reviewer-1", "REVIEWER");
    await provider.assignRole("admin-1", "ADMIN");

    assert.equal(await provider.canAccessCase("owner-1", "case-1", "MUTATE_CASE"), false);

    await provider.setCaseOwner("case-1", "owner-1");
    await provider.grantCaseAccess("case-1", "reviewer-1", ["VIEW_CASE", "REVIEW_CASE"]);

    assert.equal(await provider.canAccessCase("owner-1", "case-1", "MUTATE_CASE"), true);
    assert.equal(await provider.canAccessCase("reviewer-1", "case-1", "REVIEW_CASE"), true);
    assert.equal(await provider.canAccessCase("reviewer-1", "case-1", "RELEASE_CASE"), false);
    assert.equal(await provider.canAccessCase("admin-1", "case-1", "RELEASE_CASE"), true);
  });
});
