...
import type { CaseRecord, HlaConsensusRecord, QcGateRecord, RunArtifact, WorkflowRunRecord } from "../src/types";
...
test("Lifecycle controls", async (t) => {
  await t.test("POST /consent synchronizes case readiness", async () => {
    ...
    assert.strictEqual(withdrawResponse.body.case.caseProfile.consentStatus, "withdrawn");
  });
  ...
  await t.test("POST /restart-from-revision returns the case to READY_FOR_WORKFLOW", async () => {
    ...
    const bundleResponse = await request(app).post("/api/reference-bundles").send({
