...
- Перепроверено 2026-04-21: 504 теста (22 набора), 94.49% покрытия по строкам, 82.88% по ветвлениям, 94.11% по функциям, `npm audit --omit=dev --audit-level=high` - без уязвимостей, `npm run sbom:cyclonedx:file` пересобран.
- Архитектурная опора: 18 порт-интерфейсов, 23 адаптера (18 в памяти + 5 интеграционных), 17 состояний жизненного цикла кейса.
...
Базовый формальный срез доказательности: [docs/archive/FORMAL_EVIDENCE_REGISTER_2026-04-21.md](docs/archive/FORMAL_EVIDENCE_REGISTER_2026-04-21.md).

Migration note по изменениям control-plane от 2026-04-21: [docs/archive/reports/BREAKING_CHANGES_2026-04-21.md](docs/archive/reports/BREAKING_CHANGES_2026-04-21.md).
...
- Ведет кейс пациента через управляемый жизненный цикл (17 статусов).
...
- Поддерживает консенсус HLA из нескольких инструментов с порогами расхождения и операторским review-gate при превышении настроенного порога нерешенных конфликтов.
- Выполняет контроль качества прогонов и фиксирует решения QC.
- Сохраняет результаты ранжирования неоантигенов и дизайн конструкта, включая настраиваемые linker strategies (`ggs-flexible`, `aay-cleavage`, `direct-fusion`).
- Формирует пакет для экспертного разбора, решение комиссии, независимую финальную release-авторизацию и пакет передачи в производство.
...
- Контур доступа строится вокруг API-ключей/JWT и RBAC, при этом безопасный режим по умолчанию - запрет по умолчанию, а review и release разделены разными разрешениями.
...
| Манифестации электронных подписей и независимая final release-церемония | Реализованы частично, но не доведены до identity-bound Part 11 уровня |
...
| [`docs/INTENDED_USE.md`](docs/INTENDED_USE.md) | Формализованное intended use и границы применения |
